...
import { LogOut, QrCode, Calendar, BookOpen, BarChart2 } from 'lucide-react';
...
const StudentDashboard = ({ user, onLogout, currentView, setCurrentView }) => {
  const renderView = () => {
    switch (currentView) {
      case 'scanner':
      ...
      case 'timetable':
        return (
            <div className="max-w-7xl mx-auto">
                <button onClick={() => setCurrentView('dashboard')} className="mb-4 text-blue-600">← Back to Dashboard</button>
                <Timetable schedule={studentTimetable} userRole="student" />
            </div>
        );
      default:
        return (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                >
                    <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-6">
                        ...
                        <Timetable schedule={studentTimetable} userRole="student" isPreview={true} />
                    </div>
                </motion.div>
            </div>
          </div>
